...
to do:
    -make some sort of reset functionality.  maybe two buttons, side by side.  
    One that just resets and then one that resets with all the edited fields saved.
...
resetTabAnn='Click this button to manually return this tab\'s fields to their default states.\n'
initialLocatorsAnn='Click to create initial Locators for joint creation.\n'
FKIKAnn='Do you want an FK joint chain, an IK joint chain, or BOTH!?\n'
globalJntsList=[]
...
def createLoc_cmd(*args):
    ...
    renameEnd = cmds.rename('endJoint_loc',updatedJntName+'_loc%02i'%(updatedInptVal+2))
...
def createJnt_cmd(*args):
    ...
    jntsList=[]
    FKIKboth_check=cmds.radioButtonGrp('FKIK_both',q=True,sl=True)
    updatedJntName=cmds.textFieldGrp('inputName_grp',q=True,tx=True)
    ...
    #FKIK or both funcitonality.
    if FKIKboth_check==1:
        print "FK only!"
    elif FKIKboth_check==2:
        if mirrorCheck==True:
            halfLen=len(globalJntsList)/2
            cmds.ikHandle(n='%s_IK'%globalJntsList[0],sj=globalJntsList[0],ee=globalJntsList[halfLen-1],sol='ikRPsolver')
            cmds.ikHandle(n='%s_IK'%globalJntsList[halfLen],sj=globalJntsList[halfLen],ee=globalJntsList[-1],sol='ikRPsolver')
        else:
            cmds.ikHandle(n='%s_IK'%jntsList[0],sj=jntsList[0],ee=jntsList[-1],sol='ikRPsolver')        
    elif FKIKboth_check==3:
        kinMode={1:'IK',2:'FK'}
        print "both FK and IK!"
        if mirrorCheck==True:
            print "AAAANd mirror is checked"
        else:
            print "and mirror is NOT checked"
            for inst in range(2):
                cmds.select(jntsList[0])
                cmds.duplicate()
                for num in range(len(jntsList)):
                    currentSel=cmds.ls(sl=True)[0]
                    newJointName = cmds.rename(currentSel,jntsList[num]+'_%s'%kinMode[inst+1])
                    cmds.parentConstraint(newJointName,jntsList[num])
                    findChild=cmds.listRelatives(c=True,f=True)
                    if findChild:
...
def jointScale_cmd(*args):
    globalJntsInstance = globalJntsList
    radiusValue=cmds.floatSliderGrp('jointScaleSlider',q=True,v=True)
    for i in range(len(globalJntsInstance)):
        cmds.setAttr(globalJntsInstance[i]+".radius",radiusValue)
...
#reset Joint Tab command
def resetJointTab_cmd(*args):
    print globalJntsList
    if not globalJntsList==False:
        del globalJntsList[:]
    print globalJntsList
    #resetControlTab_cmd
    
#reset Joint Tab command
def resetControlTab_cmd(*args):
...
child1 = cmds.rowColumnLayout(numberOfRows=20,p='jointCreator')
cmds.button('initialLocators',label='Start and end joint Locator',c=startUpLocator_cmd,ann=initialLocatorsAnn)
cmds.radioButtonGrp('inputFacing_grp',label='Facing axis', labelArray3=['X', 'Y', 'Z'], numberOfRadioButtons=3,ann=radioBoxAnn,enable1=True)
...
cmds.radioButtonGrp('FKIK_both',label='FK chain?  IK chain?  Both?', labelArray3=['FK', 'IK', 'both'], numberOfRadioButtons=3,ann=FKIKAnn,enable1=True)
cmds.radioButtonGrp('FKIK_both',e=True,sl=1)
...
cmds.button('resetJointTab',label='Reset this Tab\'s fields',ann=resetTabAnn,c=resetJointTab_cmd)
child2 = cmds.rowColumnLayout(numberOfRows=20,p='jointCreator')
cmds.button('resetControlTab',label='Reset this Tab\'s fields',ann=resetTabAnn,c=resetControlTab_cmd)
cmds.tabLayout( tabs, edit=True, tabLabel=((child1, 'Joint Creation'), (child2, 'Control Creation')) )
